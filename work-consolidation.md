...
> **Verify pass — BLOCKED on account linking (2026-08-17).** The GitHub issue create/close/reopen/label/assignee UI slices (commits `673d079`→`5b2a2aa`) are built, but the **live** decode-path verification cannot run: `GET /api/github/repos` still returns 400 "GitHub account not linked" for the test account, and linking requires the interactive OAuth browser flow (it can't be driven headlessly). The client's request-shape unit tests pass and the routes are confirmed present. **To finish:** link GitHub once in the app (Account → link GitHub, or the in-list "Link GitHub" CTA), then re-run the verify pass against a real repo's issues. ⚠️ Watch for the same **response-envelope drift** the G7 pass found on `POST /api/messages` (message wrapped under `data`, results at top level) — confirm the GitHub create/update responses decode into `GitHubIssueDTO` as written.
